...
import { SearchNormal1, Add, Edit2, Trash, DocumentUpload, TicketStar, Additem, Refresh } from 'iconsax-react';
...
import ReassignStudentModal from './components/modals/reAssignStudent';
...
export default function StudentManagement() {
  ...
  const { data: schoolsDataApi, isLoading: schoolDataLoading, refetch: refetchSchools } = useGetGeneralSchool({});
  ...
  const { data: currentSchoolWristband } = useGetWristbands({});
  ...
  const reAssignStudentToDifferentSchool = useReassignStudent();
  ...
  const handleReassignStudent = (reassignmentData) => {
    reAssignStudentToDifferentSchool.mutate(reassignmentData, {
      onSuccess: () => {
        dispatch(
          openSnackbar({
            open: true,
      ...
      onError: (err) => {
        dispatch(
          openSnackbar({
            open: true,
            message: err.response?.message || 'Failed to reassign student',
            variant: 'alert',
            alert: { color: 'error' },
            close: true
          })
    ...
    <ThemeProvider>
      <Grid>
                    ...
                    <TableRow key={student.id} hover selected={isSelected(student.id)}>
                      ...
                      <TableCell align="center">
                        <Box display="flex" gap={1}>
                          {' '}
                          <Tooltip title="Reassign to Different School">
                            {' '}
                            <IconButton size="small" onClick={() => handleOpenReassignModal(student)} color="warning">
                              <Refresh size={15} />
                            </IconButton>
                          </Tooltip>{' '}
        ...
        <ReassignStudentModal
          ...
          schools={schoolsData}
          onSubmit={handleReassignStudent}
          isLoading={reAssignStudentToDifferentSchool.isPending}
        />
      </Grid>
    </ThemeProvider>
  );
